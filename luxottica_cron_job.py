    ...
    def controller(self, store: Store, brands_with_types: list[dict]) -> None:
        try:
            ...
            for brand_with_type in brands_with_types:
                # login_flag = False
                login_flag = True if '/homepage' in self.browser.current_url else False
                if not login_flag:
        ...
        if files:
            ...
            for json_d in json_data:
                ...
                for json_variant in json_d['variants']:
                    ...
                    data.append([number, frame_code, brand, sku, barcode_or_gtin, inventory_status])
    except Exception as e:
        if DEBUG: print(f'Exception in read_data_from_json_file: {e}')
        else: pass
    finally: return data

def saving_data_in_excel(data: list, excel_results_filename: str):
    try:
        workbook = Workbook()
        ...
        worksheet.cell(row=1, column=5, value='UPC')
        worksheet.cell(row=1, column=6, value='Item Status')
...
try:
    ...
    excel_results_filename = f'Results {scrape_time}.xlsx'
    ...
    if os.path.exists(start_json_filename):
        ...
        if 'brands' in json_data:
            ...
            if os.path.exists(credentails_filename):
                ...
                saving_data_in_excel(data, excel_results_filename)
